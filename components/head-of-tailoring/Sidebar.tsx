...
const Sidebar = () => {
  ...
  const markAsRead = async (id: string, message: string, link: string) => {
    try {
      ...
      if (link.includes("tailorjoblist")) {
        router.push("/head-of-tailoring/joblists/tailorjoblists/" + linking_id);
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full bg-[#262d34] flex justify-between flex-col text-[#A5A8AB] shadow-lg overflow-y-auto z-50 transition-transform duration-300 lg:w-[250px] ${
          isSidebarOpen
            ? "translate-x-0 w-3/4"
        ...
        <div>
          ...
          <div>
            ...
            <div className="relative mt-5 mx-4">
              <div
                className={`absolute left-0 top-0 h-full w-[2px] bg-[#ff6c2f] transition-opacity duration-300 ${
                  pathname === "/head-of-tailoring/jobs"
                    ? "opacity-100"
                    : "opacity-0"
                }`}
              ></div>
            ...
            <div className="">
              <ul className="space-y-1 mt-3 mb-10 px-4">
                {sidebarItems.map((item) => {
                  const isMenuOpen =
                    openMenus[item.id] || isActive(item.prefix);
