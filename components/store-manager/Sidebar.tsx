...
const sidebarItems = [
  {
    ...
    links: [
      { name: "List", href: "/store-manager/inventory" },
  ...
  const markAsRead = async (id: string, message: string, link: string) => {
    try {
      ...
      if (link.includes("orderslist")) {
        router.push("/store-manager/orders/" + linking_id);
      }
      if (link.includes("projectlist")) {
        router.push("/store-manager/joblists/projects/" + linking_id);
      }
      if (link.includes("tailorjoblist")) {
        router.push("/store-manager/joblists/tailorjoblists/" + linking_id);
      }
    } catch (error) {
