...
const sidebarItems = [
  {
    ...
    links: [
      { name: "List", href: "/client-manager/customers" },
      { name: "Create", href: "/client-manager/customers/create" },
    ],
  },
  {
    id: 2,
  ...
  const markAsRead = async (id: string, message: string, link: string) => {
    try {
      ...
      if (link.includes("projectlist")) {
        router.push("/client-manager/joblists/projects/" + linking_id);
      }
      if (link.includes("tailorjoblist")) {
        router.push("/client-manager/joblists/tailorjoblists/" + linking_id);
      }
    } catch (error) {
