...
import { FaChartBar, FaChartLine, FaMoneyBillWave, FaPercent } from "react-icons/fa"
...
export default function PaymentChart() {
  ...
  const [selectedType, setSelectedType] = useState<"year" | "month">("year") // Simplified to year/month for now
  ...
  return (
    <Card className="w-full mb-8 rounded-2xl shadow-xl border border-orange-100">
      ...
      <CardContent className="p-6">
        {loading ? (
          ...
          >
            <LoadingSpinner />
            <p className="text-gray-600 mt-4 font-medium">Loading chart data...</p>
          </motion.div>
        ) : error ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-10">
            <p className="text-gray-500">Please select specific month.</p>
          </motion.div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <Card className="p-4 flex items-center justify-between bg-blue-50 border-blue-200">
                <div>
