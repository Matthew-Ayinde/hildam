  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data.status === "success") {
        ...
        const processedNotifications = data.data.map((notif: Notification) => ({
          ...notif,
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-6xl">
      ...
      {loading ? (
        ...
        >
          <HiOutlineExclamation className="mx-auto text-red-500 mb-2" size={32} />
          <h3 className="text-lg font-medium text-red-800 mb-1">Error loading notifications</h3>
          <p className="text-red-600">{error}</p>
          <Button onClick={fetchNotifications} className="mt-4 bg-red-600 hover:bg-red-700">
            Try again
          </Button>
        </motion.div>
